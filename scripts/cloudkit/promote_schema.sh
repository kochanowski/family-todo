...
log_file="$tmp_dir/cktool-promote-production.log"

render_ckdb "$SCHEMA_JSON" "$ckdb_file"

echo "CloudKitSchema: promoting schema to production for container=$CONTAINER_ID"
...
if [[ -f "$log_file" ]]; then
  cp "$log_file" "$REPO_ROOT/cloudkit/artifacts/cktool-promote-production.log"
fi

echo "CloudKitSchema: production schema promote finished."
